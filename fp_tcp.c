...
struct tcp_sig* fingerprint_tcp( u8 to_srv, struct packet_data* pk, struct packet_flow* f ) {
  ...
  /* Detect packets generated by p0f-sendsyn; they require special
    ...
    if ( to_srv ) {
        obj = start_observation(f->sendsyn ? "sendsyn probe" : "syn", 4, 1, f);
    }else {
        obj = start_observation(f->sendsyn ? "sendsyn response" : "syn+ack", 4, 0, f);
    }

    tcp_find_match(to_srv, sig, 0, f->syn_mss);

    if ( (m = sig->matched) ) {
        OBSERVF( obj, (m->class_id == -1 || f->sendsyn) ? "app" : "os", "%s%s%s", fp_os_names[m->name_id], m->flavor ? " " : "", m->flavor ? m->flavor : (u8*)"");
    }else {
        add_observation_field( obj, "os", NULL );
    }

    if ( m && m->bad_ttl ) {
        OBSERVF( obj, "dist", "<= %u", sig->dist);
    }else {
        if ( to_srv ) {
    ...
    add_observation_field( obj, "params", dump_flags(pk, sig) );
...
void check_ts_tcp( u8 to_srv, struct packet_data* pk, struct packet_flow* f ) {
    ...
    if ( ffreq < MIN_TSCALE || ffreq > MAX_TSCALE ) {
        /* Allow bad reading on SYN, as this may be just an artifact of IP sharing or OS change. */
        if ( pk->tcp_type != TCP_SYN ) {
             if (to_srv) {
                f->cli_tps = -1; 
            }else {
    ...
    obj = start_observation( "uptime", 2, to_srv, f );
    ...
    OBSERVF( obj, "uptime", "%u days %u hrs %u min (modulo %u days)", (up_min / 60 / 24), (up_min / 60) % 24, up_min % 60, up_mod_days );
    OBSERVF( obj, "raw_freq", "%.02f Hz", ffreq );
}
